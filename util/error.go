...
func TransformAWSError(err error) error {
	...
	// check the error is wrapped with errors.WithStackTrace(), then we can't check the actual error is aws. So handling the situation here
	...
	if err := commonErr.Unwrap(err); err != nil {
		...
		if errors.As(err, &awsErr) && awsErr.ErrorCode() == "RequestCanceled" {
			return ErrContextExecutionTimeout
		}
	}
