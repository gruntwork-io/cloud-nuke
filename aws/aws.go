...
func GetAllResources(
	...
	for _, region := range targetRegions {
		...
		registeredRegionalResources := GetRegisteredRegionResource(cloudNukeSession, region)
		for _, resource := range registeredRegionalResources.Resources {
			if IsNukeable(resource.ResourceName(), resourceTypes) {
				...
				if len(identifiers) > 0 {
					resourcesInRegion.Resources = append(resourcesInRegion.Resources, resource)
				}
			}
	...
	if collections.ListContainsElement(targetRegions, GlobalRegion) {
		...
		registeredGlobalResources := GetRegisteredGlobalResources(session)
		for _, resource := range registeredGlobalResources.Resources {
			if IsNukeable(resource.ResourceName(), resourceTypes) {
				start := time.Now()
				identifiers, err := resource.GetAndSetIdentifiers(configObj)
				if err != nil {
				...
				// Only append if we have non-empty identifiers
				if len(identifiers) > 0 {
					globalResources.Resources = append(globalResources.Resources, resource)
				}
			}
		}
