...
// Returns a formatted string of Snapshots snapshot ids
	...
	for _, snapshotID := range snapshotIds {
		...
		if err != nil {
			...
			telemetry.TrackEvent(commonTelemetry.EventContext{
				EventName: "Error Nuking EBS Snapshots",
			}, map[string]interface{}{
				"region": *session.Config.Region,
			})
		} else {
			deletedSnapshotIDs = append(deletedSnapshotIDs, snapshotID)
			logging.Logger.Debugf("Deleted Snapshots: %s", *snapshotID)
		}
	}

	logging.Logger.Debugf("[OK] %d Snapshots(s) terminated in %s", len(deletedSnapshotIDs), *session.Config.Region)
	return nil
}
