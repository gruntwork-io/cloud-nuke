...
func TestConfigRDSSnapshots_IncludeNames(t *testing.T) {
	configFilePath := "./mocks/rdsSnapshots_include_names.yaml"
	configObj, err := GetConfig(configFilePath)
	...
	if len(configObj.RDSSnapshots.IncludeRule.NamesRE) == 0 {
		assert.Fail(t, "ConfigObj should contain RDS snapshot names regexes, %+v\n", configObj)
	}

	return
}

func TestConfigRDSSnapshots_ExcludeNames(t *testing.T) {
	configFilePath := "./mocks/rdsSnapshots_exclude_names.yaml"
	configObj, err := GetConfig(configFilePath)
	...
	if len(configObj.RDSSnapshots.ExcludeRule.NamesRE) == 0 {
		assert.Fail(t, "ConfigObj should contain RDS snapshot names regexes, %+v\n", configObj)
	}
...
func TestConfigRDSSnapshots_FilterNames(t *testing.T) {
	configFilePath := "./mocks/rdsSnapshots_filter_names.yaml"
	configObj, err := GetConfig(configFilePath)
